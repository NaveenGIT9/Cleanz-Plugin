...
export class DashboardPanel {
  ...
  private static _htmlCache: string | undefined; // cached once, reused on every reveal/reset
  private readonly _panel: vscode.WebviewPanel;
  private readonly _context: vscode.ExtensionContext;
  private _disposables: vscode.Disposable[] = [];

  private _lastConfig: RunConfig | undefined;
  private _currentProc: import('child_process').ChildProcess | undefined;
  private _lastCwd: string = '';
  private _preRunHead: string = '';
  private _abortDidHardReset: boolean = false;
  private _lastBatchItems: BatchItemUI[] = [];
  ...
  private _queueLog(level: string, text: string) {
  ...
  public static preWarm(context: vscode.ExtensionContext) {
  ...
  public queueRun(config: RunConfig) {
  ...
  private async _handleMessage(message: { command: string; [key: string]: unknown }) {
    switch (message.command) {
      ...
      case 'startRun': {
        const config = message.config as RunConfig;
        this._lastConfig = config;
      ...
      case 'abort': {
        ...
        if (proc?.pid) {
          ...
          const runKill = (cmd: string, args: string[], ms = 3000): Promise<void> =>
            new Promise((resolve) => {
              try {
      ...
      case 'namespacePurge': {
        await this._namespacePurge(message.namespace as string);
        break;
      }
  ...
  private async _startCleanzRun(config: RunConfig) {
    ...
    this._preRunHead = '';
    exec('git rev-parse HEAD', { cwd }, (_err: unknown, stdout: string) => {
      ...
      try {
        ...
        const quick = allKnown
          ...
          .map((i) => ({
            name: i.n,
            type: typeMap[i.t],
            op: (i.a ?? '').toLowerCase() === 'full' ? ('FULL' as const) : ('ADD' as const),
            status: 'pending',
            statusLabel: 'Pending',
            refs: '—',
          }));
        ...
        const isDeployable = (i: { a?: string }) => {
          if (!i.a) return true;
          const op = i.a.toLowerCase();
          return !op.startsWith('retrieve') && !op.startsWith('delete');
        };
        const whitelistTypes: Record<string, string> = {
          ApexClass: 'ApexClass',
          ApexPage: 'VF Page',
          CustomField: 'Field',
          CustomObject: 'Object',
        ...
        const whitelistGroups: Record<string, string[]> = {};
        for (const i of raw) {
        ...
        this.postMessage({ command: 'log', level: 'err', text: `[pre-parse] ${(e as Error).message ?? e}` });
      }
    }
    ...
    const setItemStatus = (name: string, status: string, statusLabel: string, refs?: string) => {
    ...
    const dispatchCsvRefCounts = (csvText: string) => {
          ...
          if (noActiveRun && !this._abortDidHardReset && fs.existsSync(abortCsvPath)) {
            try {
              ...
              await new Promise<void>((resolve) => {
                exec('git log --oneline -1 HEAD', { cwd, timeout: 3000 }, (_err: unknown, stdout: string) => {
                  let gitRepo = 0;
                  if (stdout) {
        ...
        if (code === 127 && !this._autoRebuildInProgress) {
          ...
          try {
            ...
            if (pluginPath) {
              ...
              await new Promise<void>((resolve) => {
                exec('npx oclif manifest', { cwd: pluginPath, timeout: 30_000 }, () => resolve());
              });
              rebuilt = true;
  ...
  private async _exportReport(_data: unknown) {
    ...
    await new Promise<void>((resolve) => {
      exec('git rev-parse --abbrev-ref HEAD', { cwd: this._lastCwd }, (_err: unknown, stdout: string) => {
    ...
    const isAdvisory = (e: string) => e.startsWith('[OBJ_PERM_WARN] ') || e.startsWith('[PSG_UPDATING_WARN] ');
    const isHint = (e: string) => e.startsWith('[HINT] ');
...
export type RunConfig = {
  jsonPath: string;
  org: string;
  mode: 'validate' | 'dryrun' | 'namespace-purge';
  repoSweep: boolean;
  verbose: boolean;
  namespace?: string;
};
